...
parser.add_argument('--experiment', default='tiny', type=str, required=False, help='(default=%(default)s)')
parser.add_argument('--approach', default='OCMMM', type=str, required=False, help='(default=%(default)s)')
...
parser.add_argument('--input_size', type=str, default=[3, 64, 64], help='(default=%(default)s)')
parser.add_argument('--buffer_size', type=int, default=10000, help='(default=%(default)s)')
...
def rot_inner_all(x):
  ...
  #  imshow(torchvision.utils.make_grid(a))
    ...
    R[num:2*num] = torch.cat((s1_1.unsqueeze(2), s2.unsqueeze(2)), dim=2).reshape(num,3, 64, 64).permute(0,1,3,2)
    R[3*num:] = torch.cat((s1.unsqueeze(2), s2_2.unsqueeze(2)), dim=2).reshape(num,3, 64, 64).permute(0,1,3,2)
    R[2*num:3*num] = torch.cat((s1_1.unsqueeze(2), s2_2.unsqueeze(2)), dim=2).reshape(num,3, 64, 64).permute(0,1,3,2)
...
print('Load data...')
oop=16
data, taskcla, inputsize, Loder, test_loder = dataloader.get_tinyimagenet_100(seed=args.seed)
print('Input size =', inputsize, '\nTask info =', taskcla)
buffero = buffer(args).cuda()
Basic_model = b_model(200).cuda()
Optimizer = Adam(Basic_model.parameters(), lr=0.001, betas=(0.9, 0.99),weight_decay=1e-4)
Basic_model, Optimizer = amp.initialize(Basic_model, Optimizer)
...
with torch.no_grad():
    ...
    resize_crop = TL.RandomResizedCropLayer(scale=resize_scale, size=[64, 64, 3]).cuda()
...
buffer_per_class =7
for run in range(1):
    rank=torch.arange(0,100)#randperm(len(Loder))
        ...
        if i==0:
            ...
            for epoch in range(1):
                ...
                for batch_idx, (x, y) in enumerate(train_loader):
                  ...
                  #  rot_sim_labels = torch.cat([labels1 + 1000 * i for i in range(oop)], dim=0)
...
#
                    ...
                    #feature_map=Basic_model.f_train(t)
                    #u=Basic_model.simclr(feature_map)
                    feature_map, u = Basic_model.forward(t, is_simclr=True)
                    #pre_u_feature=Previous_model.f_train(images_pair_r)
                    #pre_u = Previous_model.simclr(pre_u_feature)
                    ...
                    pre_feature_map_out_r = normalize(pre_u_feature)#.detach()

                    images_out = u[:images_pair.shape[0]]
                    images_out_r = u[images_pair.shape[0]:]
                    pre_u = normalize(pre_u)#.detach()#torch.cat((images_out_r,pre_u),dim=0)
                   ...
                   # simclr_now=normalize(u[images_pair.shape[0]:images_pair.shape[0]+images1_r.shape[0]])
                    ...
                    rot_sim_labels = torch.cat([y.cuda()+ 1000 * i for i in range(oop)],dim=0)
            #        rot_sim_labels1 = torch.cat([torch.arange(0,y.shape[0]).cuda() +10*i for i in range(4)], dim=0)
             #       rot_sim_labels_r1 = torch.cat([torch.arange(0,mem_y.shape[0]).cuda()+10*i for i in range(4)], dim=0)
                    rot_sim_labels_r = torch.cat([mem_y.cuda()+ 1000 * i for i in range(oop)],dim=0)
                  ...
                  #  id3 = torch.randperm(num1)[0]
                    ...
                    #sim_matrix = torch.zeros((simclr.shape[0], simclr.shape[0])).cuda()
                    #sim_matrix_r = torch.zeros((simclr_r.shape[0], simclr_r.shape[0])).cuda()
                    #sim_matrix_r_pre = torch.zeros((simclr_pre.shape[0], simclr_pre.shape[0])).cuda()
                    ...
                    #sim_matrix += torch.matmul(simclr, feature_map_out[:, id1_2:id1_2 + size].t())
                    #sim_matrix_r += torch.matmul(simclr_r,
                     #                           feature_map_out_r[:, id2_2:id2_2 + size].t())
                   # sim_matrix/=2
                    #sim_matrix_r/=2
                    #sim_matrix_r_pre = torch.matmul(simclr_r,
                     #                                     pre_feature_map_out_r[:, id3:id3 + size].t())
                    # pdb.set_trace()
             ...
             #       loss_sim_mix2= Supervised_NT_xent(sim_matrix_mix,labels=rot_sim_labels_mix2,temperature=0.07)
                  ...
                  #  y_label = Basic_model.linear(y_feature)
                   ...
                   # y_label_1 = Basic_model.linear(hidden_pred)
                    ...
                    loss = 1 * F.cross_entropy(y_label, mem_y) + lo1++1*F.mse_loss(y_label_pre[:,:2*task_id], y_label[:,:2*task_id]) #+1*F.mse_loss(y_feature, logits)+ 1*F.mse_loss(y_feature, Pre_y_feature)
